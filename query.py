...
#It is a table that contains only foreign keys which link, (associate),
#many tables together (many to many). The relationship type that it 
#manages is the "many to many" relationship.
...
def get_model_info(year):
    ...
    query = db.session.query(Model.name, Brand.name, Brand.headquarters).filter(Model.year == 1960).all()
...
def get_brands_summary():
    ...
    for brand in query:
        brands.setdefault(brand[0], []).append((brand[1], brand[2]))

    for key, value in brands.iteritems():
        print key
        for brands_info in value:
            print brands_info[0], brands_info[1]
...
def search_brands_by_name(mystr):
    ...
    mystr = '%' + mystr + '%'
    query = Brand.query.filter(Brand.name.ilike(mystr)).all()
